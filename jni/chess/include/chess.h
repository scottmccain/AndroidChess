...
void DisplayChessBoard(POSITION);
void DisplayChessBoardFile(FILE *, POSITION);
...
void History(TREE *RESTRICT, int, int, int, int, int*, int);
...
int NextMove(TREE *RESTRICT, int, int, int);
...
int SearchParallel(TREE *RESTRICT, int, int, int, int, int, int, int);
...
//int jni_fseek(FILE *, int, int);
//int jni_fprintf(FILE *, char *, ...);
...
#  define HistoryIndex(m) ((Piece(m) << 6) + To(m))
/*
  side = side to move
...
#define SPEAK native_sound

#define _printf native_send
