...
int Option(TREE * RESTRICT tree) {
	 ...
	 */
	    ...
	    if (nargs != 6) {
	      _printf("usage:  adaptive NPS hmin hmax pmin pmax\n");
	      return 1;
	    }
	    if (nargs > 1) {
	      ...
	      do {
	        if (OptionMatch("time", args[1])) {
	          ...
	          if (display_options & 1)
	            _printf("display time for moves\n");
	          if (display_options & 2)
	            _printf("display variation when it changes.\n");
	          if (display_options & 4)
	            _printf("display variation at end of iteration.\n");
	          if (display_options & 8)
	            _printf("display basic search stats.\n");
	          if (display_options & 16)
	            _printf("display extended search stats.\n");
	          if (display_options & 32)
	            _printf("display ply-1 moves as they are searched.\n");
	          if (display_options & 64)
	            _printf("display move numbers in variations.\n");
	          if (display_options & 128)
	            _printf("display general messages.\n");
	          if (display_options & 256)
	            _printf("display ply-1 node counts every iteration.\n");
	          if (display_options & 512)
	            _printf("display ply-1 moves and evaluations.\n");
	        } else
	          break;
	        return 1;
	      } while (0);
	 ...
	 */
	    ...
	    if (nargs > 1) {
	      ...
	      if (new_hash_size < 64 * 1024) {
	        _printf("ERROR.  Minimum hash table size is 64K bytes.\n");
	        return 1;
	      }
	      ...
	      if (!trans_ref) {
	        _printf("AlignedRemalloc() failed, not enough memory.\n");
	        exit(1);
	      }
	 ...
	 */
	    ...
	    if (nargs > 1) {
	      new_hash_size = atoiKMB(args[1]);
	      if (new_hash_size < 64 * 1024) {
	        _printf("ERROR.  Minimum phash table size is 64K bytes.\n");
	        return 1;
	      }
	 ...
	 */
	    ...
	    if (nargs > 1) {
	      ...
	      if (new_hash_size < 16 * 1024) {
	        _printf("ERROR.  Minimum pawn hash table size is 16K bytes.\n");
	        return 1;
	      }
	 ...
	 */
	    ...
	    if (!helpfile) {
	      _printf("ERROR.  Unable to open \"crafty.hlp\" -- help unavailable\n");
	      return 1;
	    }
	    if (nargs > 1) {
	      while (1) {
	        ...
	        if (!readstat) {
	          _printf("Sorry, no help available for \"%s\"\n", args[1]);
	          fclose(helpfile);
	          return 1;
	        }
	        if (buffer[0] == '<') {
	 ...
	 */
	    ...
	    if (nargs < 4) {
	      _printf("usage:  level <nmoves> <stime> <inc>\n");
	      return 1;
	    }
	 ...
	 */
	    ...
	    if (nargs > 2) {
	      _printf("usage:  linelength <n>\n");
	      return 1;
	    }
	    if (nargs == 2)
	      line_length = atoi(args[1]);
	    _printf("line length set to %d.\n", line_length);
	  }
	/*
	 ************************************************************
	 *                                                          *
	 ...
	 */
	    ...
	    if (list > 4) {
	      _printf("usage:  list AK|B|GM|IM|P|SP +name1 -name2 etc\n");
	      return 1;
	    }
	    nargs -= 2;
	    targs += 2;
	    if (nargs) {
	      while (nargs) {
	        if (targs[0][0] == '-') {
	          ...
	          _printf("error, name must be preceeded by +/- flag.\n");
	        nargs--;
	        targs++;
	      }
	    } else {
	 ...
	 */
	    ...
	    if (nargs != 1 && nargs != 6) {
	      _printf("usage:  lmr <min> <max> <depth bias> <move bias> <scale>\n");
	      return 1;
	    }
	    if (nargs > 1) {
	 ...
	 */
	    ...
	    nargs = ReadParse(buffer, args, " \t=");
	    if (nargs < 3) {
	      _printf("usage:  input <filename> title\n");
	      return 1;
	    }
	 ...
	 */
	    ...
	    if (nargs < 2) {
	      _printf("usage:  log on|off|n [filename]\n");
	      return 1;
	    }
	    if (!strcmp(args[1], "on")) {
	      int id;
	 ...
	 */
	    ...
	    if (nargs > 1) {
	      if (!strcmp(args[1], "tournament")) {
	        ...
	        _printf("use 'settc' command if a game is restarted after Crafty\n");
	        _printf("has been terminated for any reason.\n");
	        ...
	        _printf("usage:  mode normal|tournament|match\n");
	        mode = normal_mode;
	        book_weight_learn = 1.0;
	        book_weight_freq = 1.0;
	        book_weight_eval = 0.5;
	 ...
	 */
	    ...
	    _printf("tellicsnoalias kibitz Hello from Crafty v%s! (%d cpus)\n",
	 ...
	 */
	    ...
	    if (nargs > 3) {
	      _printf("usage:  null <min> <divisor>\n");
	      return 1;
	    }
	    if (nargs > 1) {
	 ...
	 */
	    ...
	    if (nargs < 2) {
	      _printf("usage:  operator <seconds>\n");
	      return 1;
	    }
	 ...
	 */
	    ...
	    if (nargs < 2) {
	      _printf("usage:  otime <time(unit=.01 secs))>\n");
	      return 1;
	    }
	 ...
	 */
	    ...
	    if (nargs < 2) {
	      _printf("usage:  output long|short\n");
	      return 1;
	    }
	 ...
	 */
	    if (nargs == 3 && !strcmp(args[1], "save")) {
	      ...
	      if (!file) {
	        _printf("ERROR.  Unable to open %s for writing\n", args[2]);
	        return 1;
	      }
	      _printf("saving to file \"%s\"\n", filename);
	 ...
	 */
	    ...
	    if (personality_packet[param].size == 0) {
	      if (nargs > 3) {
	        _printf("this eval term requires exactly 1 value.\n");
	        return 1;
	      }
	 ...
	 */
	    ...
	    nargs = ReadParse(buffer, args, " \t=");
	    if (nargs < 2) {
	      _printf("usage:  bookpath|perspath|logpath|tbpath <path>\n");
	      return 1;
	    }
	    if (!strchr(args[1], '(')) {
	...
	#define PERF_CYCLES 4000000
	    ...
	    _printf("generated %d moves, time=%.2f seconds\n",
	        (int) (tree->last[1] - tree->last[0]) * PERF_CYCLES, time_used);
	    _printf("generated %d moves per second\n",
	    ...
	    time_used =
	        ((float) clock_after - (float) clock_before) / (float) CLOCKS_PER_SEC;
	    _printf("generated/made/unmade %d moves, time=%.2f seconds\n",
	        (int) (tree->last[1] - tree->last[0]) * PERF_CYCLES, time_used);
	    _printf("generated/made/unmade %d moves per second\n",
	        (int) (((float) (PERF_CYCLES * (tree->last[1] -
	                        tree->last[0]))) / time_used));
	  }
	/*
	 ...
	 */
	    ...
	    time_used =
	        ((float) clock_after - (float) clock_before) / (float) CLOCKS_PER_SEC;
	    _printf("total moves=%" PRIu64 "  time=%.2f\n", total_moves, time_used);
	  }
	/*
	 ************************************************************
	 *                                                          *
	 ...
	 */
	    ...
	    if (thinking || pondering)
	      return 2;
	    if (nargs < 2) {
	      _printf("usage:  ponder off|on|<move>\n");
	      return 1;
	    }
	    if (!strcmp(args[1], "on")) {
	      ponder = 1;
	 ...
	 */
	    ...
	    int rd;
	    if (nargs < 3) {
	      _printf("usage:  rating <Crafty> <opponent>\n");
	      return 1;
	    }
	 ...
	 */
	    ...
	    if (thinking || pondering)
	      return 2;
	    if (nargs < 2) {
	      _printf("usage:  reset <movenumber>\n");
	      return 1;
	    }
	    ponder_move = 0;
	    last_mate_score = 0;
	 ...
	 */
	      ...
	      if (move) {
	        ...
	        _printf("ERROR!  move %s is illegal\n", buffer);
	        break;
	      }
	    ...
	    moves_out_of_book = 0;
	    _printf("NOTICE: %d moves to next time control\n",
	 ...
	 */
	    ...
	    if (nargs > 1) {
	      ...
	      _printf("type \"exit\" to terminate.\n");
	      read_input = stdin;
	    }
	    if (!append) {
	      InitializeChessBoard(tree);
	 ...
	 */
	    ...
	    do {
	      if (read_input == stdin) {
	        if (game_wtm)
	          _printf("read.White(%d): ", move_number);
	        else
	          _printf("read.Black(%d): ", move_number);
	        fflush(stdout);
	      }
	 ...
	 */
	    do {
	      ...
	      if (move) {
	        if (read_input != stdin) {
	          _printf("%s ", OutputMove(tree, move, 0, game_wtm));
	 ...
	 */
	    ...
	    if (nargs > 1) {
	      if (!(output_file = fopen(args[1], "w"))) {
	        _printf("unable to open %s for write.\n", args[1]);
	        return 1;
	      }
	    }
	 ...
	 */
	    ...
	    if (thinking || pondering)
	      return 2;
	    if (nargs < 4) {
	      _printf("usage:  settc <wmoves> <wtime> <bmoves> <btime>\n");
	      return 1;
	    }
	 ...
	 */
	    ...
	    if (nargs < 2) {
	      _printf("usage:  skill <1-100>\n");
	      return 1;
	    }
	    if (skill != 100) {
	      _printf("ERROR:  skill can only be changed one time in a game\n");
	    } else {
	      skill = atoi(args[1]);
	      if (skill < 1 || skill > 100) {
	        _printf("ERROR: skill range is 1-100 only\n");
	        skill = 100;
	      }
	 ...
	 */
	    ...
	    if (nargs < 2) {
	      _printf("usage:  smpgroup <threads>\n");
	      return 1;
	    }
	 ...
	 */
	    ...
	    if (thinking || pondering)
	      return 2;
	    if (nargs < 2) {
	      _printf("usage:  test <filename> [exitcnt]\n");
	      return 1;
	    }
	    if (nargs > 2)
	 ...
	 */
	    ...
	    if (nargs < 3) {
	      _printf("usage:  timebook <percentage> <move span>\n");
	      return 1;
	    }
	 ...
	 */
	    ...
	    if (nargs < 2) {
	      _printf("usage:  usage <percentage>\n");
	      return 1;
	    }
	 ...
	 */
	    ...
	    switch (i) {
	      case 7:
	        ...
	        break;
	      default:
	        _printf("sorry, only wild7 implemented at present\n");
	        break;
	    }
	  }
	/*
